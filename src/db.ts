...
// Service role: este é um bot backend (nunca roda no navegador), então pode
// ignorar RLS com segurança — não expor essa chave em nenhum client-side.
const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);

// O schema (tabelas `entries` e `ai_usage`, com os CHECK de área/categoria)
// agora vive em migrations do Supabase, não mais criado/migrado por este
// arquivo — a antiga `migrarSchemaEntries` (troca de `talhao` por
// `area`/`local` no SQLite) não se aplica mais.
