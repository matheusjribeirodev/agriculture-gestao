...
export type ResultadoInterpretacao =
  ...
  | { tipo: "gerar_pdf"; periodo: "mes_atual" | "mes_passado"; area?: Area }
...
const SYSTEM_PROMPT = `Você é um assistente de gestão para o produtor de uma propriedade rural (com foco em café, mas cobrindo a propriedade toda), conversando por WhatsApp. Data de hoje: ${hoje()}.
...
export async function interpretarMensagem(
  ...
  if (resultado.tipo === "registrar") {
    const dados = corrigirCategoriaInvalida(EntradaSchema.parse(resultado.chamada.input));
    return { tipo: "registrar", dados };
  }

  if (resultado.tipo === "gerar_pdf") {
    const { periodo, area } = GerarPdfSchema.parse(resultado.chamada.input);
    return { tipo: "gerar_pdf", periodo: periodo ?? "mes_atual", area };
  }
